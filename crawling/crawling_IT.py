...
BASE_LIST_URL = "https://kabc.dongguk.edu/content/list?itemId=ABC_IT&lang=ko"
BASE_HOST = "https://kabc.dongguk.edu"
TREE_ALL_URL = "https://kabc.dongguk.edu/content/list?itemId=ABC_IT&lang=ko"
...
def extract_all_volume_links_from_tree(soup: BeautifulSoup) -> List[str]:
    ...
    carriers = soup.select(
        '[data-url^="?itemId=ABC_IT&cate=bookName&depth=1&upPath=Z&dataId="]')
    if not carriers:
        return []
...
def extract_volume_links_from_expanded_page(soup: BeautifulSoup, parent_id: str) -> List[str]:
    ...
    carriers = soup.select(
        f'[data-url*="?itemId=ABC_IT&cate=bookName&depth=2&upPath=Z&dataId={parent_id}"]'
    )
...
def build_tree_expand_url(parent_data_id: str) -> str:
    ...
    return urljoin(
        BASE_HOST,
        f"/content/list?itemId=ABC_IT&lang=ko&cate=bookName&depth=2&upPath=Z&dataId={parent_data_id}",
    )
...
def build_tree_root_open_url() -> str:
    ...
    return urljoin(
        BASE_HOST,
        "/content/list?itemId=ABC_IT&lang=ko&cate=bookName&depth=1&upPath=Z",
    )
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KABC ABC_IT 번역/주석 크롤러")
    parser.add_argument(
        "--list-url",
        default=BASE_LIST_URL,
        help="목록 URL (기본: https://kabc.dongguk.edu/content/list?itemId=ABC_IT&lang=ko)",
    )
    default_output = "/home/work/paper/buddha/crawling/output/kabc_ABC_IT.jsonl"
    parser.add_argument(
        "--output",
    ...
    default_resume = "/home/work/paper/buddha/crawling/output/kabc_ABC_IT.resume.json"
